...
class SEResNet(torch.nn.Module):
    def __init__(self, n_class: int, scale: int):
        ...
        self.se1_2 = SELayer(20, reduction=4, attend_dim="freq_timewise")

        self.block2_1 = TransitionBlock(8*scale, 12*scale, stride=(2, 1), dilation=(1, 2), temp_pad=(0, 2))
        self.block2_2 = BroadcastedBlock(12*scale, dilation=(1, 2), temp_pad=(0, 2))

        self.se2_1 = SELayer(12*scale, reduction=4, attend_dim="chan")
        self.se2_2 = SELayer(10, reduction=4, attend_dim="freq_timewise")
        ...
        self.se3_2 = SELayer(5, reduction=4, attend_dim="freq_timewise")
        ...
        self.se4_2 = SELayer(5, reduction=4, attend_dim="freq_timewise")
    ...
    def forward(self, x):
        ...
        out = self.block1_1(out)
        out = self.block1_2(out)
        out = self.se1_1(out).transpose(2, 3)
        out = self.se1_2(out).transpose(2, 3)

        out = self.block2_1(out)
        out = self.block2_2(out)
        out = self.se2_1(out).transpose(2, 3)
        out = self.se2_2(out).transpose(2, 3)

        out = self.block3_1(out)
        out = self.block3_2(out)
        out = self.block3_3(out)
        out = self.block3_4(out)
        out = self.se3_1(out).transpose(2, 3)
        out = self.se3_2(out).transpose(2, 3)

        out = self.block4_1(out)
        out = self.block4_2(out)
        out = self.block4_3(out)
        out = self.block4_4(out)
        out = self.se4_1(out).transpose(2, 3)
        out = self.se4_2(out).transpose(2, 3)
